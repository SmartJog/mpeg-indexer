...
static int saf_write_eof_marker(StreamContext *saf)
{
    ByteIOContext *pb = &saf->opb;
...
static int write_index(StreamContext *stcontext)
{
    ...
    put_le64(&indexpb, 0x766978004BABDAEDLL);
    ...
    put_buffer(&indexpb, index_buf, index_size);
    put_flush_packet(&indexpb);
    url_fclose(&indexpb);
    return 0;
}
static int saf_write_trailer(StreamContext *saf)
{
    if (write_index(saf) < 0)
        return -1;
    saf_write_eof_marker(saf);
    return 0;
}
...
{
//    int drop = 0,  hours = 0, minutes = 0, seconds = 0, frames = 0;
    if (st) {
        if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
            uint8_t *buf = pkt->data;
            uint32_t c = -1, d = -1;
            int i, j;
            for(i = 0; i < pkt->size - 4 && d != 0x100; i++) {  //&& c != 0x1B8 
                if (c != 0x1B8){
                    c = (c<<8) + buf[i]; 
                    j = i+1;
                }
                d = (d<<8) + buf[i];
//                printf("%d buf[i]: %x\t c : %x\t d : %x\n",i, buf[i], c, d);
            } 
//            printf ("c : %x\n", c);
            if (c == 0x1B8) { // found GOP header => I-frame follows with temporal reference of 0 
//                drop = !!(buf[i] & 0x80);
                ...
                printf("GOP timecode :\t%02d:%02d:%02d:%02d\n", last_key_frame->hours, last_key_frame->minutes, last_key_frame->seconds, last_key_frame->frames);
                (*kf)++;
            }
            if (d == 0x100) { // found picture start code
                printf("Pic Start Code\n");
                uint32_t temp_ref=0;
                uint32_t frame_type;
